...
function closestPlayer()
	for i, v in pairs(config.regions) do -- get the availabile players in the regions that you're allowed to check
		local targetablePlayers = getPlayersInRegion:Invoke(v)--game.Players:GetPlayers()
...
function DangerCalculator()
	
	-- The boss should look at different things such as damage output of the players, health, bloodlines, achievements, etc to determine a max dangerscore.
	-- if the players have roughly the same health and dangerscore then the boss will target them evenly 
	-- if there is a player who is significantly stronger than the others then the boss will target them
	-- if there are two strong players of equal MAX strength but one is lower health then the boss will target the one with lower health 
	-- enemies that are below the bosses lower threshold for dangerscore will be eliminated at the lowest priority because they'll be deemed incapable of damaging the boss.
	
	local dangerScore = 0
	local target = ""
	local weight = 0
	local chances = {} -- the players within the range of the boss will be ranked by their "danger" score and then the boss will weigh which person is most dangerous and prioritize killing them first!
	for i, v in pairs(config.regions) do
		local players = getPlayersInRegion:Invoke(v)
